...
# %% [markdown]
# Rearrange df to convert into LaTeX table
...
for engine in engines_to_make:
    d = df_agg.copy()
    d = d[(d["ocr_engine"] == engine) | (d["ocr_engine"] == "None")]

    # Drop vision-only, GPT-chosen and combined OCR runs for the drop first page tables
    d = d[
        ~d["method"].apply(
            lambda s: "all" in s
            or "chosen" in s
            or sum(e in s for e in ocr_engines) > 1
            or "pbp" in s
            or "per_page" in s
        )
    ]
    ...
    caption = (
        f"Relative performance of MLLMs and prompting strategies compared to the baseline \\textbf{{{ocr_strs[engine]}}} OCR engine on \iam{{}}. "
